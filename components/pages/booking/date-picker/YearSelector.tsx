...
export const YearSelector = ({ onSelect, selectedYear }: Props) => {
  ...
  return (
    <div
      ref={listRef}
      className="mt-8 w-full grid grid-cols-4 gap-2 pr-1 max-h-48 overflow-y-auto text-sm [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-neutral-300 [&::-webkit-scrollbar-track]:rounded-lg [&::-webkit-scrollbar-thumb]:bg-neutral-600 [&::-webkit-scrollbar-thumb]:rounded-full dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-400"
    >
      {years.map((year) => (
        <button
          key={year}
          ...
          className={`px-2 py-2 rounded-lg text-center transition-colors duration-300 ease-in-out cursor-pointer text-small font-inter
            ${year === selectedYear
