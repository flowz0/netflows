...
import { sendConsultationFormAction } from "@/actions/email";
...
export default function MultiStepForm() {
  ...
  const onBtnBlur = (e: React.FocusEvent<HTMLButtonElement>) => {
    ...
    const error = validateField(name as keyof FormProps, value);
  ...
  const validateStep1 = () => {
    const requiredFields: (keyof FormProps)[] = [
  ...
  return (
    <form ref={formRef} className="border-1 border-black75 bg-black5 mt-28 py-12 px-6 rounded-2xl animate-drop-shadow sm:py-20 sm:px-12">
      {/* Stepper */}
